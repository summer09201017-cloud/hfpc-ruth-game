...
> ✅ **2026-07-07:tts 管線建成+朗讀鐵則落地+母體死碼清完**:
> ① 照 paul 抄 `src/speak.js`/`src/ttsFix.js`/`scripts/gen-tts.mjs`;卡片四關過關自動朗讀(CardGame 對齊 paul 版,
> 含 🔊 再聽一次與「滿分只算計分題」修正)、gleaning 過關朗讀得 2:17;**5 句曉臻 mp3 烤畢**(manifest 5 句,
> vite precache 加 mp3,17 entries)——不再機器聲。
> ② **死碼清完**:src/minigames/ 的 jonah/sling/arkbuild/arkpairs 四引擎+六個母體 Demo 元件+其 main.jsx 路由+
> sync-jonah/sling-*/_ark-beard-* 腳本+package.json sync:jonah+play-ark-*.bat 全數移除;MiniGameModal 瘦身成
> cards+gleaning+elijah 三線;smoke-test 嵌入契約改守 gleaning。npm test 全綠、test:offline 55 項全綠。
> 🔜 待做:卡片四關敘述文案(AI 草擬)送牧者審(引文已 cuv 核)→ 過審後大廳點卡。
...
**(2026-07-07 死碼清理後,本 repo 的小遊戲只有三線)** A station triggers a mini-game via its `minigame` field (decoupled from `type`; the dedicated tile uses `type: "challenge"`):
- `minigame: { cards: "<key>", winPoints }` → 純 React 卡片流程關(`src/minigames/cards/`,路得四關 ruthChoice/threshingFloor/gateRedemption/ruthFinale)。
- `minigame: { engine: 'gleaning', winPoints }` → 🌾 拾麥穗蒙恩(`src/minigames/gleaning/`,簽名關,elijah 換皮;嵌入契約 `new Game(canvas,{embed,winPoints,onComplete})`+`boot()`/`destroy()`;smoke-test 的嵌入契約檢查守這支)。
- `minigame: { engine: 'elijah', winPoints }` → gleaning 的母引擎,保留可用(`?demo=elijah-action`)。
**朗讀(2026-07-07 起)**:固定句一律 `speakScripture(text,{ref})`(`src/speak.js`,mp3 優先)——新句子加進 `scripts/tts-verses.json` 後跑 `node scripts/gen-tts.mjs` 烤曉臻 mp3(累加式,跑到「新產 0」;鐵則:絕不用 speakText 唸固定句)。
母體的 jonah/sling/arkbuild/arkpairs 引擎、六個母體 Demo、sync:jonah 管線已全數移除(歷史段落已一併刪除)。
...
`MiniGameModal.jsx` mounts the canvas (boot is deferred to a 「開始挑戰」 click for the audio-unlock gesture) and on completion calls `resolveStation({ minigameWon, minigameScore })` — the engine scores it in `resolve` (pure, result injected like a quiz answer). 2026-07-07 瘦身後只分派 cards / gleaning / elijah 三線。
